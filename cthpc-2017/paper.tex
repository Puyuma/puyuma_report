...
The lane segment detection is performed before pose estimation. The ROI(Region of interest) is used to increase the accuracy by eliminate the region. Canny edge detection, HSV color thresholding and Hough transform are used to find the lane segments.
...
We can generate the lane pose for every segments according to the lane geometry. The lane pose $d$ and $phi$ represent the car lateral displacement and orientation angle. we later feed this different poses into the histogram filter for a better prediction of the lane pose.

\subsubsection{Histogram Filter}

The histogram filter works like a vote box. We first filter out extreme value vote by the \textbf{mode}. We then calculate the \textbf{mean} of the rest of the data. Higher vote pose will have higher chance to dominate the result.

\subsubsection{Controller}

We use the differential wheeled type car, which there are two independent motors on two side. The car can turn by changing the speed rate between two motors, therefore it doesn't requires a steering mechanics. We use the \textbf{PID Control} algorithm to fix the orientation and lateral displacement.
